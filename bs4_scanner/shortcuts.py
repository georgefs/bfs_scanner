...
def simple_imgur_handler(callback=mirror):
    return ("blockquote.imgur-embed-pub", blockquote_imgur_extractor(callback), False)  # noqa


def simple_instagram_handler(callback=mirror):
    return ("blockquote.instagram-media", blockquote_instagram_extractor(callback), False)  # noqa


def simple_text_handler(callback=mirror):
    return (navigablestring_matcher(), quick_extractor(lambda info, e: callback(info, unicode(e).strip()) or None), False)  # noqa
